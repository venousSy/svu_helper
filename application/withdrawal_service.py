...
class WithdrawalService:
    ...
    async def request_withdrawal(
        ...
        # 3. Deduct balance (raises InsufficientBalanceError atomically if short)
        await self._repo.deduct_balance(user_id, amount)

        # 4. Record date (prevents second request today)
        await self._repo.record_withdrawal_date(user_id)

        # 5. Build and persist the request
        ...
        # 6. Fetch data for the audit report
        ...
        remaining_balance = (user.balance if user else 0.0) - amount
