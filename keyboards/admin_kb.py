...
def get_admin_dashboard_kb() -> types.InlineKeyboardMarkup:
    ...
    builder.row(
        types.InlineKeyboardButton(
            text="📑 قائمة المشاريع الكاملة", callback_data="view_all_master"
        )
    )
    builder.row(
        types.InlineKeyboardButton(
            text="📊 مشاريع قيد الانتظار", callback_data="view_pending"
        )
    )
    builder.row(
        types.InlineKeyboardButton(
            text="✅ مشاريع مقبولة/جارية", callback_data="view_accepted"
        )
    )
    builder.row(
        types.InlineKeyboardButton(text="📜 سجل المشاريع", callback_data="view_history")
    )
    builder.row(
        types.InlineKeyboardButton(
            text="💰 سجل المدفوعات", callback_data="view_payments"
        )
    )
    builder.row(
        types.InlineKeyboardButton(
...
def get_pending_projects_kb(pending_projects):
    ...
    for item in pending_projects:
        ...
        builder.row(
            types.InlineKeyboardButton(text=btn_text, callback_data=f"manage_{p_id}")
        )
    return builder.as_markup()
...
def get_accepted_projects_kb(accepted_projects):
    ...
    for item in accepted_projects:
        ...
        builder.row(
            types.InlineKeyboardButton(
                text=btn_text, callback_data=f"manage_accepted_{p_id}"
            )
        )  # Finish
    return builder.as_markup()
...
def get_manage_project_kb(p_id):
    ...
    builder.row(
        types.InlineKeyboardButton(
            text="💰 إرسال عرض", callback_data=f"make_offer_{p_id}"
        )
    )  # Send Offer
    builder.row(
        types.InlineKeyboardButton(text="❌ رفض", callback_data=f"deny_{p_id}")
    )  # Reject
    builder.row(
        types.InlineKeyboardButton(text="⬅️ رجوع", callback_data="view_pending")
    )
    return builder.as_markup()
...
def get_payment_verify_kb(proj_id):
    ...
    builder.row(
        types.InlineKeyboardButton(
            text="✅ تأكيد الدفع", callback_data=f"confirm_pay_{proj_id}"
        ),  # Confirm Pay
        types.InlineKeyboardButton(
            text="❌ رفض الدفع", callback_data=f"reject_pay_{proj_id}"
        ),  # Reject Pay
    )
    return builder.as_markup()
...
def get_payment_history_kb(payments):
    ...
    for pay in recent_payments:
        ...
        builder.row(
            types.InlineKeyboardButton(
                text=f"📄 عرض الإيصال #{p_id}", callback_data=f"view_receipt_{p_id}"
            )
        )
